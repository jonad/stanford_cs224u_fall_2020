...
class TorchGloVeDataset(torch.utils.data.Dataset):
    def __init__(self, X_log: torch.FloatTensor, weights:torch.FloatTensor) -> None:
        """
...
class TorchGloVeModel(nn.Module):
    def __init__(self, n_words:int, embed_dim:int) -> None:
        super().__init__()
    ...
    def forward(self, X_log:torch.FloatTensor, idx:torch.LongTensor) -> torch.FloatTensor:
        """
        Parameters
        ----------
...
class TorchGloVeLoss(nn.Module):
    ...
    def forward(self, diffs:torch.FloatTensor, weights:nn.Parameter) -> torch.FloatTensor:
